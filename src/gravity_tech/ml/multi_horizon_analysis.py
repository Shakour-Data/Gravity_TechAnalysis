...
class MultiHorizonTrendAnalyzer:
    ...
    def _generate_recommendation(
        ...
        if pattern == MarketPattern.STRONG_UPTREND:
            return f"🚀 STRONG BUY - All horizons bullish"
        
        elif pattern == MarketPattern.STRONG_DOWNTREND:
            return f"⛔ STRONG SELL - All horizons bearish"
        
        elif pattern == MarketPattern.BUY_THE_DIP:
            if horizon == '3d':
                return f"💎 BUY THE DIP - Short-term correction, long-term bullish"
            else:
                return f"📈 HOLD/BUY - Long-term trend positive"
        
        elif pattern == MarketPattern.SELL_THE_RALLY:
            if horizon == '3d':
                return f"💰 TAKE PROFIT - Short-term rally, long-term bearish"
            else:
                return f"📉 SELL - Long-term trend negative"
        
        elif pattern == MarketPattern.TREND_REVERSAL:
            return f"🔄 TREND REVERSAL - Short and long-term divergence"
        
        elif pattern == MarketPattern.CONSOLIDATION:
            return f"⏸️ WAIT - Market consolidating"
